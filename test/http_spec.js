...
describe('$http', function () {
    ...
    it('support functions as header values', function () {
    ...
    it('ignore header function value when null/undefined', function () {
    ...
    it('allows settings withCredentials', function () {
        $http({
            method: 'POST',
            url: 'http://teropa.info',
            data: 42,
    ...
    it('allows settings withCredentials from defaults', function () {
    ...
    it('does not choke response resembling JSON but not valid', function () {
        var response;
        $http({
            method: 'GET',
            url: 'http://teropa.info'
    ...
    it('allows substituting param serializer', function () {
        ...
        expect(requests[0].url).toBe('http://teropa.info?a=42lol&b=43lol');
    });
    ...
    describe('JQ-like param serialization', function () {
        ...
        it('is possible', function () {
            ...
            expect(requests[0].url).toBe('http://teropa.info?a=42&b=43');
        });

        it('use square brackets in arrays', function () {
            $http({
                url: 'http://teropa.info',
                params: {
                    a: [42, 43]
            ...
            expect(requests[0].url).toBe('http://teropa.info?a%5B%5D=42&a%5B%5D=43');
        });

        it('use square brackets in objects', function () {
            $http({
                url: 'http://teropa.info',
                params: {
                    a: {b: 42, c: 43}
            ...
            expect(requests[0].url).toBe('http://teropa.info?a%5Bb%5D=42&a%5Bc%5D=43');
        });
        ...
        it('supports nesting in objects', function () {
            ...
            expect(requests[0].url).toBe('http://teropa.info?a%5Bb%5D%5Bc%5D=42');
        });
        ...
        it('appends array indexes when items are objects', function () {
            ...
            expect(requests[0].url).toBe('http://teropa.info?a%5B0%5D%5Bb%5D=42');
        });
    });
    ...
    it('allows intercepting requests', function () {
        ...
        $http.get('http://tepora.info', {params: {}});
        $rootScope.$apply();
        expect(requests[0].url).toBe('http://tepora.info?intercepted=true');
    });
    ...
    it('allows intercepting responses', function () {
        ...
        var response;
        $http.get('http://tepora.info').then(function (r) {
            response = r;
        });
        $rootScope.$apply();
    ...
    it('allows attaching success handlers', function () {
        ...
        $http.get('http://tepora.info').success(function (d, s, h, c) {
            data = d;
            status = s;
            headers = h;
            config = c;
    ...
    it('allows attaching error handlers', function () {
        var data, status, headers, config;
        $http.get('http://tepora.info').error(function (d, s, h, c) {
            data = d;
            status = s;
            headers = h;
            config = c;
