...
ASTCompiler.prototype.stringEscapeRegex = /[^\sa-zA-Z0-9]/g;
    ...
    switch (ast.type) {
        case AST.Program:
            _.forEach(_.initial(ast.body), function (stmt) {
                this.state[this.state.computig].body.push(this.recurse(stmt), ';');
            }.bind(this));
            this.state[this.state.computig].body.push(
...
ASTCompiler.prototype.nextId = function (skip) {
    ...
    if (!skip) {
        this.state[this.state.computig].vars.push(id);
    }
    return id;
};
